...
sub close {
	...
	for my $func ( @{ $self->{can_do_list} } ) {
		my $sleeping = $sleepers->{$func};
		delete $sleeping->{$self};

		my $new_sleepers_list;
		for my Gearman::Server::Client $c ( @{ $sleepers_list->{$func} } ) {
			next unless $c;
			if ( $sleeping->{$c} ) {
				push( @{$new_sleepers_list}, $c );
			}
		}

		if ($new_sleepers_list) {
			$self->{server}->{sleepers_list}->{$func} = $new_sleepers_list;
		} else {
			delete $self->{server}->{sleepers_list}->{$func};
		}

		delete $sleepers->{$func} unless %$sleeping;
	}
